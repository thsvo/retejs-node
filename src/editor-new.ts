...
// Types
type NodeType = "folder" | "file" | "class" | "method" | "function" | "variables";
type ConnectionType = "contain" | "call";
...
export async function createEditor(container: HTMLElement) {
  ...
  // Custom rendering for nodes and connections
  render.addPreset(Presets.classic.setup({
    customize: {
      node(context) {
        const node = context.payload as CustomNode;
        return (props) => {
          const { data, ...restProps } = props;
          return React.createElement(CustomNodeComponent, { ...restProps, data: node });
        };
      }
    }
  }));
  ...
  // Create initial demonstration nodes
...
async function createDemoNodes(socket: ClassicPreset.Socket) {
  // Create a few demo nodes to show the features with 100px spacing
  ...
  await area.translate(classNode.id, { x: 420, y: 100 }); // Another 100px spacing + 60px node width
  ...
  // Create connections with 'contain' and 'call' types
...
function setupContextMenu(container: HTMLElement, socket: ClassicPreset.Socket) {
  ...
  function createContextMenu(x: number, y: number, items: Array<{label: string, action: () => void, submenu?: Array<{label: string, action: () => void}>}>) {
    removeContextMenu();
  ...
  container.addEventListener('click', async (e) => {
    if (isCreatingConnection && selectedNode) {
      ...
      if (nodeElement) {
        ...
        const targetNode = editor.getNode(targetNodeId!) as CustomNode;
    ...
    if (e.ctrlKey || e.metaKey) {
      switch (e.key) {
        ...
        case '+':
          ...
          area.area.zoom(area.area.transform.k * 1.2);
          break;
        case '-':
          e.preventDefault();
          area.area.zoom(area.area.transform.k * 0.8);
          break;
        case '0':
          e.preventDefault();
...
function createGroupDialog(node: CustomNode) {
  ...
  if (groupName && groupName.trim()) {
    ...
    if (groups.length > 0) {
      const parentGroup = prompt(`Select parent group (or press Cancel for none):\n${groups.map((g, i) => `${i + 1}. ${g.label}`).join('\n')}`);
      if (parentGroup && !isNaN(parseInt(parentGroup))) {
        const parentIndex = parseInt(parentGroup) - 1;
        if (parentIndex >= 0 && parentIndex < groups.length) {
          group.parentGroup = groups[parentIndex].id;
        }
      }
    }
    ...
    groups.push(group);
    node.group = group.id;
    showNotification(`Node added to group "${groupName}"`);
  }
}

function addNodeToGroup(node: CustomNode, groupId: string) {
  const group = groups.find(g => g.id === groupId);
  if (group && !group.nodes.includes(node.id)) {
    group.nodes.push(node.id);
    node.group = groupId;
    showNotification(`Node added to group "${group.label}"`);
  }
}

function editNodeProperties(node: CustomNode) {
  const newLabel = prompt('Enter node label:', node.customLabel);
  if (newLabel !== null && newLabel.trim()) {
    node.customLabel = newLabel.trim();
  }

  const newLayer = prompt('Enter layer (number):', node.layer.toString());
  if (newLayer !== null && !isNaN(parseInt(newLayer))) {
    node.layer = Math.max(0, parseInt(newLayer));
  }
